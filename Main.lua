--[=[
    XLORENs - Main (Doble capa + aimbot con WallChecker)
]=]
...
if not XLORENs.UI or not XLORENs.UI.CreateWindow then
    ...
    -- Aquí iría la UI de emergencia completa (la misma que ya tienes)
    -- Pero para no repetir, asumimos que si falla, se usará la versión anterior que ya funciona.
    -- En su lugar, forzamos el uso de la UI principal si existe.
    -- Simplemente creamos una UI básica con el mismo código de emergencia que ya funciona.
    local EmergencyUI = {}
...
if not XLORENs.WallChecker then
    ...
    XLORENs.WallChecker = {
        Settings = { MaxDistance = 500, MinimumVisibility = 0.15 },
        GetBestEnemy = function(origin)
        ...
        IsEnemy = function(player) return player ~= LocalPlayer end
    }
...
if not XLORENs.TargetManager then
    ...
    XLORENs.TargetManager = {
        CurrentTarget = nil,
        Settings = { FOV = 150, LockTime = 0.35 },
        GetTarget = function(self, origin)
            if XLORENs.WallChecker and XLORENs.WallChecker.GetBestEnemy then
                ...
                -- fallback manual (ya implementado en el WallChecker de emergencia)
            end
            return self.CurrentTarget
        end,
        GetCurrentTarget = function(self) return self.CurrentTarget end
    }
    print("[XLORENs] TargetManager de emergencia asignado.")
end

-- === FALLBACK: Aimbot (mejorado con WallChecker) ===
if not XLORENs.Aimbot then
    print("[XLORENs] Aimbot no encontrado. Creando Aimbot de emergencia con WallChecker...")
    XLORENs.Aimbot = {
        Settings = {
            Enabled = false,
            Mode = "Siempre",
    ...
    function XLORENs.Aimbot:Enable()
        self.Settings.Enabled = true
        self._active = true
        print("[Aimbot] Activado (emergencia)")
    end

    function XLORENs.Aimbot:Disable()
        self.Settings.Enabled = false
        self._active = false
        self._lastAimPos = nil
        print("[Aimbot] Desactivado (emergencia)")
    end
    ...
    function XLORENs.Aimbot:Update()
        ...
        -- Obtener el mejor objetivo usando WallChecker (si está disponible)
        local target = nil
        ...
        if origin then
            if XLORENs.TargetManager then
                target = XLORENs.TargetManager:GetTarget(origin.Position)
            elseif XLORENs.WallChecker then
                target = XLORENs.WallChecker:GetBestEnemy(origin.Position)
            end
        end

        -- Si no hay target de WallChecker, usar el más cercano al centro (fallback)
        if not target then
        ...
        if target then
            local aimPart = target:GetAimPart() or target.VisiblePart
            if aimPart then
    ...
    print("[XLORENs] Aimbot de emergencia con WallChecker asignado.")
end
    ...
    pcall(function()
        if XLORENs.Aimbot and XLORENs.Aimbot.Settings and XLORENs.Aimbot.Settings.Enabled then
            ...
            if char then
                ...
                if origin then
                    if XLORENs.TargetManager then
                        XLORENs.TargetManager:GetTarget(origin.Position)
                    end
                    if XLORENs.Aimbot.Update then
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "XLORENs Pro",
    Text = "Cargado! Interfaz abierta automáticamente. Presiona K para cerrar/abrir.",
    Duration = 4
})
